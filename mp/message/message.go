...
func getMixMessage(body io.ReadCloser) (*MixMessage, error) {
	rawMsg, err := ioutil.ReadAll(body)
	if err != nil {
		return nil, err
	}
...
func Handle(w http.ResponseWriter, r *http.Request) {
	mixMsg, err := getMixMessage(r.Body)
	if err != nil {
		fmt.Println("1:", err)
		return
	}
	...
	respByte, err := xml.Marshal(response[0].Interface())
	if err != nil {
		fmt.Println("2:", err)
		return
	}
	fmt.Println(methodName, string(respByte))
	w.Write(respByte)
	return
}
